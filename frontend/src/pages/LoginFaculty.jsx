...
function LoginFaculty() {
  ...
  function submit(e) {
    ...
    if (!email && !password) {
      return toast.error("please all feild are reuired");
    }
    if (!email) {
    ...
    fetch("http://localhost:8080/api/v1/user/faculty-login", requestOptions)
      .then((response) => response.json())
      .then((result) => {
        if (result.token) {
          localStorage.setItem("faculty-token", result.token);
      ...
      <div className="  flex justify-center items-center h-screen  ">
        <div className=" w-full mx-4 max-w-md shadow p-5 bg-white border rounded-2xl  ">
          <div className="text-center flex gap-3 flex-col items-center">
            <h1 className="text-4xl">faculty login</h1>
          </div>
          <form action="" className="mt-4">
            <div className="flex flex-col mt-4">
              ...
              <input
                ...
                className="shadow p-2 focus:border-blue-400 outline-none rounded-xl mt-1 border bg-gray-50"
              />
            </div>
            <div className="flex flex-col mt-4">
